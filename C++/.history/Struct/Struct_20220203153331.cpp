...
int main(void)
{
    ...
    Jung_prop = CalcProperty(Jung.saving,Jung.load);

    cout << "정경재의 재산은 적금 " <<  Jung.saving << "원에 이율 " << Jung.load << "원을 제외한 총 " << Jung_prop << "원입니다." << endl;

    Jung_prop = CalcProperty(&Jung);
    cout << "정경재의 재산은 적금 " <<  Jung.saving << "원에 이율 " << Jung.load << "원을 제외한 총 " << Jung_prop << "원입니다." << endl;

    Prop kim = InitProp();
    int kim_prop = CalcProperty(&kim);
    cout << "김의 재산은 적금 " <<  kim.saving << "원에 이율 " << kim.load << "원을 제외한 총 " << kim_prop << "원입니다." << endl;
